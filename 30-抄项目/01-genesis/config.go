...
var assetMap = map[string]*File{
	"/config.json": &File{
		name:    "/config.json",
		hash:    "64323134323366623934613738623134666435333137343234636636356232316635366633393964",
		modTime: time.Unix(1538728420, 315578292),
		data:    []byte("\x7b\x0a\x20\x20\x22\x41\x22\x3a\x22\x31\x32\x33\x22\x2c\x0a\x20\x20\x22\x42\x22\x3a\x22\x33\x34\x35\x22\x0a\x7d"),
	},
}
...
// Hash returns the SHA1 hash of the file's data.
...
func AssetNameWithHash(name string) string {
	if f := AssetFile(name); f != nil {
		return JoinNameHash(f.Name(), f.Hash()[:HashSuffixLength])
	}
	return name
}

// HashSuffixLength is the length of the hash suffix used by AssetNameWithHash()
var HashSuffixLength = 7
