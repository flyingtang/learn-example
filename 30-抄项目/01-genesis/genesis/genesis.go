...
func (enc *Encoder) writeFileType() error {
	var buf bytes.Buffer
	fmt.Fprintln(&buf, `// File represents a single embedded asset file.`)
	...
	fmt.Fprintln(&buf, ``)
	fmt.Fprintln(&buf, `// Data returns the raw embedded data for the file.`)
...
func (enc *Encoder) writeAssetFuncs() error {
	var buf bytes.Buffer
	fmt.Fprintln(&buf, `// Asset returns the raw data given an embedded filename.`)
	fmt.Fprintln(&buf, `// Returns nil if the asset cannot be found.`)
	...
	fmt.Fprintln(&buf, `}`)
	fmt.Fprintln(&buf, ``)
	fmt.Fprintln(&buf, `// AssetFile returns the File object given an embedded filename.`)
	fmt.Fprintln(&buf, `// Returns nil if the asset cannot be found.`)
	fmt.Fprintln(&buf, `func AssetFile(name string) *File {`)
	fmt.Fprintln(&buf, `	if f := assetMap[name]; f != nil {`)
	fmt.Fprintln(&buf, `		return f`)
	fmt.Fprintln(&buf, `	}`)
	fmt.Fprintln(&buf, `	file, hash := SplitNameHash(name)`)
	fmt.Fprintln(&buf, `	if f := assetMap[file]; f != nil && strings.HasPrefix(f.hash, hash) {`)
	fmt.Fprintln(&buf, `		return f`)
	fmt.Fprintln(&buf, `	}`)
	fmt.Fprintln(&buf, `	return nil`)
	fmt.Fprintln(&buf, `}`)
	fmt.Fprintln(&buf, ``)
	fmt.Fprintln(&buf, `// AssetNames returns a sorted list of all embedded asset filenames.`)
	fmt.Fprintln(&buf, `func AssetNames() []string {`)
	fmt.Fprintln(&buf, `	return assetNames`)
	fmt.Fprintln(&buf, `}`)
	fmt.Fprintln(&buf, ``)
	fmt.Fprintln(&buf, `// AssetNameWithHash returns the asset name with a short hash before the extension.`)
	fmt.Fprintln(&buf, `// Returns name if the asset cannot be found.`)
	fmt.Fprintln(&buf, `func AssetNameWithHash(name string) string {`)
	fmt.Fprintln(&buf, `	if f := AssetFile(name); f != nil {`)
	fmt.Fprintln(&buf, `		return JoinNameHash(f.Name(), f.Hash()[:HashSuffixLength])`)
	fmt.Fprintln(&buf, `	}`)
	fmt.Fprintln(&buf, `	return name`)
	fmt.Fprintln(&buf, `}`)
	fmt.Fprintln(&buf, ``)
	fmt.Fprintln(&buf, `// HashSuffixLength is the length of the hash suffix used by AssetNameWithHash()`)
	fmt.Fprintln(&buf, `var HashSuffixLength = 7`)
	fmt.Fprintln(&buf, ``)
...
func (enc *Encoder) writeFileSystem() error {
	var buf bytes.Buffer
	fmt.Fprintln(&buf, `// FileSystem returns a http.FileSystem implementation of the embedded files.`)
	fmt.Fprintln(&buf, `// This can be used with http.FileServer().`)
	...
	fmt.Fprintln(&buf, `// FileServer returns a handler that supports accessing assets with embedded hashes.`)
	...
	fmt.Fprintln(&buf, `func (f *httpFile) Size() int64                { return int64(len(f.File.Data())) }`)
...
func (enc *Encoder) writeHashFuncs() error {
	var buf bytes.Buffer
	fmt.Fprintln(&buf, `// JoinNameHash returns a filename with hash embedded before the extension.`)
	fmt.Fprintln(&buf, `func JoinNameHash(name, hash string) string {`)
	fmt.Fprintln(&buf, `	return strings.TrimSuffix(name, path.Ext(name)) + "-" + hash + path.Ext(name)`)
	fmt.Fprintln(&buf, `}`)
	fmt.Fprintln(&buf, ``)
	fmt.Fprintln(&buf, `// SplitNameHash separates name into its original filename and hash parts.`)
	fmt.Fprintln(&buf, `func SplitNameHash(name string) (file, hash string) {`)
	fmt.Fprintln(&buf, `	ext := path.Ext(name)`)
	fmt.Fprintln(&buf, `	base := strings.TrimSuffix(name, ext)`)
	fmt.Fprintln(&buf, `	file = hashSuffixRegex.ReplaceAllLiteralString(base, "") + path.Ext(name)`)
	fmt.Fprintln(&buf, `	if s := hashSuffixRegex.FindString(base); s != "" {`)
	fmt.Fprintln(&buf, `		hash = s[1:]`)
	fmt.Fprintln(&buf, `	}`)
	fmt.Fprintln(&buf, `	return file, hash`)
	fmt.Fprintln(&buf, `}`)
	fmt.Fprintln(&buf, ``)
	fmt.Fprintln(&buf, `// HasNameHash returns true if name contains an embedded hash.`)
	fmt.Fprintln(&buf, `func HasNameHash(name string) bool {`)
	fmt.Fprintln(&buf, `	return hashSuffixRegex.MatchString(strings.TrimSuffix(name, path.Ext(name)))`)
	fmt.Fprintln(&buf, `}`)
	fmt.Fprintln(&buf, ``)

	fmt.Fprintln(&buf, `var hashSuffixRegex = regexp.MustCompile("-[0-9a-f]{7,}$")`)
